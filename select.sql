...
from 
	tb_alunos
where 
	interesse in('Jogos', 'Informática');

-- like operator (% = any characters in text) qualquer conjunto que tenha (_ is a positional) qualquer conjunto que tenha na ordem especificada
-- _ é quantitativo, % não
select 
	nome
from 
	tb_alunos
where nome like '_riel' or nome like '%eg';
...
-- agregation functions MIN, MAX, AVG
...
-- ------------ GROUP BY ----------- : responde a perguntas como (total de alunos por curso e etc)
    
-- agrupando por interesse:
select 
	interesse, count(*)
from 
	tb_alunos
...
from
	tb_alunos
where
	idade = (select min(idade) from tb_alunos);


-- HAVING (filer over the result of group by) -------  A mini complex query HERE
select
	min(idade) as minimo
from 
	tb_alunos
group by 
	idade
having
	minimo = (select min(idade) from tb_alunos)
    or 
    minimo > 40; 
