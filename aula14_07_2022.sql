...
alter table curso add column idCurso int first;
...
cursopreferido int,
primary key (id),
foreign key(cursopreferido)references curso (idCurso))default charset= utf8;

alter table aluno add foreign key (cursopreferido) references curso (idCurso);
...
select  count(distinct(nacionalidade)) from aluno;


select count(*) from curso where carga > 30;
...
-- maior numero de aula
select max(totalAulas) from curso where ano = 2016;

-- minimo numero de aulas
select min(totalAulas) from curso where ano = 2016;
...
select sum(carga) from curso;
 ...
 -- funcoes ano, mes e dia
 select year(nascimento) ,month(nascimento), day(nascimento) from aluno;